...
- 信息渲染逻辑位于 `src/bubble-view.ts`，会根据设置拼装路径、创建时间、修改时间和自定义字段。
- 当前头部信息样式采用“单外层容器 + 内部多条信息项”的结构：外层负责圆角、边框和阴影，内部信息项负责紧凑排布与截断显示。
- 当前头部信息默认纵向排列，每条信息独占一行；当某一行内容宽度超过可用区域时，外层容器应保留横向滑动能力，避免内容被直接裁切且无法右滑查看。
