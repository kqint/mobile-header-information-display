...
## 项目理解
- 这是一个 Obsidian 移动端插件，在 `.view-header` 中插入当前笔记的元信息展示容器。
- 信息渲染逻辑位于 `src/bubble-view.ts`，当前只根据设置展示路径、创建时间和修改时间，不再渲染自定义显示项。
- 当前头部信息样式采用“单外层容器 + 内部多条信息项”的结构：外层负责圆角、边框和阴影，内部信息纵向排列，每条信息独占一行。
- 当某一行头部信息宽度超过可用区域时，外层容器应保留横向滑动能力，避免内容被直接裁切且无法右滑查看。
- 设置页位于 `src/settings.ts`，最前面提供语言切换下拉框，支持 `auto`、`en`、`zh-CN`，自定义显示项区域只保留“敬请期待”占位说明。
- 仓库文档使用双语 README：`README.md` 和 `README.zh-CN.md`，当前采用一致的章节框架，包含简介、功能、安装、使用、配置、开发、国际化贡献和许可说明。
